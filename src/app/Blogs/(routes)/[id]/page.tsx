  ...
  useEffect(() => {
    if (blog?.presignedContentUrl) {
        ...
        import("mammoth")
          .then((mammoth) => {
            ...
            fetch(proxyUrl)
              .then((response) => {
                console.log(
                  "📥 Fetched Word document via proxy, status:",
                  response.status,
                );
                if (!response.ok) {
                  throw new Error(
              ...
              .then((arrayBuffer) => {
                ...
                return (mammoth as any).convertToHtml(
      ...
      if (blog && !blog.presignedContentUrl) {
        ...
        setError(
          "Content URL not available. The backend must provide a valid presigned URL.",
        );
        setContentLoading(false);
      }
    }
        ...
        {error && !loading && (
          <div className="max-w-4xl mx-auto px-4 py-12">
            <div className="bg-red-50 border border-red-200 rounded-lg p-8 text-center">
              ...
              <Link
                href="/blogs"
                className="inline-block bg-gradient-to-t from-[#1F3A52] to-[#4682B8] text-white px-6 py-2 rounded-full hover:shadow-lg transition"
              >
                Back to Blogs
              </Link>
            </div>
        ...
        {!loading && blog && (
          <article className="max-w-4xl mx-auto px-4 py-12">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M14 10h4.764a2 2 0 011.789 2.894l-3.646 7.23a2 2 0 01-1.789 1.106H5a2 2 0 01-2-2V8a2 2 0 012-2h1.657a2 2 0 011.414.586l2.828-2.829a2 2 0 112.828 2.829l-.828.828h2.172a2 2 0 012 2v.286a2 2 0 01-2 2H12v4z"
                    />
                  </svg>
            ...
            <div className="mt-12 pt-8 border-t">
              <Link
                href="/blogs"
                className="inline-block bg-gradient-to-t from-[#1F3A52] to-[#4682B8] text-white px-8 py-3 rounded-full hover:shadow-lg transition"
              >
                ← Back to Blogs
              </Link>
            </div>
