...
export default function PartnerForm() {
  ...
  const onSubmit = (data: FormValues) => {
    // NOTE: files from react-hook-form will be FileList objects
    console.log('submit data:', data)
    // Example: create FormData and POST to your API route
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <label className="block">
          ...
          <input {...register('phone', { required: true, pattern: /^[0-9]{10}$/ })} placeholder="10-digit mobile number"
            className="w-full border border-gray-200 rounded-md px-3 py-2 text-sm" />
          {errors.phone && <span className="text-xs text-red-500">Enter valid 10-digit number</span>}
        </label>
      </div>

      <label className="block">
        <div className="text-sm text-gray-600 mb-1">Email Address *</div>
        <input {...register('email', { required: true })} placeholder="your.email@example.com"
          className="w-full border border-gray-200 rounded-md px-3 py-2 text-sm" />
      ...
      <label className="block">
        ...
        <input {...register('businessName', { required: true })} placeholder="Enter your business or organization name"
          className="w-full border border-gray-200 rounded-md px-3 py-2 text-sm" />
        {errors.businessName && <span className="text-xs text-red-500">Required</span>}
      </label>

      <div className="mt-4">
        <div className="text-sm text-gray-700 mb-2">Services Offered * (Select all that apply)</div>
      ...
      <label className="block mt-4 max-w-[220px]">
        ...
        <input {...register('experience')} placeholder="e.g., 5"
          className="w-full border border-gray-200 rounded-md px-3 py-2 text-sm" />
      </label>
      ...
      <label className="block mt-3">
        ...
        <input {...register('coverage')} placeholder="e.g., Within 50km of Mumbai city center (optional)"
          className="w-full border border-gray-200 rounded-md px-3 py-2 text-sm" />
      </label>

      {/* Additional */}
      <div className="text-sm font-medium text-gray-800 mt-6 mb-3">Additional Details</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <label>
          ...
          <input {...register('gst')} placeholder="22AAAAA0000A1Z5"
            className="w-full border border-gray-200 rounded-md px-3 py-2 text-sm" />
        </label>
      </div>
      ...
      <div className="pt-4">
        ...
        <p className="text-center text-xs text-gray-400 mt-2">We will review and contact you within 48 hours.</p>
      </div>
    </form>
  )
}
